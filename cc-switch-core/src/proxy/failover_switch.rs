...
impl FailoverSwitchManager {
    ...
    async fn do_switch(
        ...
        if let Ok(Some(_provider)) = self.db.get_provider_by_id(provider_id, app_type) {
            log::debug!("[FO-003] Provider switched successfully for {app_type}");
            // Note: Live backup update would be handled by ProxyService in TUI version
        }

        log::info!("[Failover] Successfully switched {app_type} to {provider_name}");
