...
fn emit_auto_sync_status_updated(_status: &str, _error: Option<&str>) {
    // GUI event emission removed - TUI version logs status instead
    log::debug!("[WebDAV] Auto-sync status: {}, error: {:?}", _status, _error);
}
